...
# from flask_wtf.csrf import CSRFProtect   # optional; leave disabled if you prefer
...
# Make small helper available to templates (avoids current_app in Jinja)
...
# -----------------------------------------------------------------------------
# Health & HEAD handling
#   - Render pings with HEAD /. We short-circuit those to avoid template work.
# -----------------------------------------------------------------------------
...
@app.before_request
def _short_circuit_head_for_health_and_root():
    if request.method == "HEAD" and request.path in ("/", "/healthz"):
        # Return immediately: avoids running view logic / template rendering.
        return ("", 200)
...
with app.app_context():
    ...
    if not has_root:
        ...
        def _fallback_index():  # name intentionally different than "index"
            try:
