...
def validate_environment():
    ...
    if missing_vars and os.environ.get('FLASK_ENV') == 'production':
        ...
        # Don't exit in production, just log the warnings
...
app.config['PERMANENT_SESSION_LIFETIME'] = 7200  # Auto logout after 2 hours (7200 seconds)
...
# Configuration constants - Use environment variables only
...
with app.app_context():
    ...
    try:
        ...
        # For production, we should still start the app but log the error
        if os.environ.get('FLASK_ENV') == 'production':
            logger.error("Application starting without database connection - some features will not work")
        else:
            # In development, we might want to continue with SQLite fallback
